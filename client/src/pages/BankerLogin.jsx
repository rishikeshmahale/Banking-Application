...
const BankerLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      dispatch(login(data));
      // navigate(`/${data.role}/all-customers`);
      navigate("/banker/all-customers");
    } catch (error) {
  ...
  return (
    <div className="mt-15 min-h-screen flex items-center justify-center w-full">
      <div className="bg-white shadow-md rounded-3xl px-8 py-6 w-full sm:w-[27vw]">
        <h1 className="text-2xl font-bold text-center mb-4">Welcome Banker!</h1>
        <form onSubmit={handleLogin}>
          ...
          >
            Not a Banker? Login as a Customer
          </Link>
        </form>
      </div>
    </div>
